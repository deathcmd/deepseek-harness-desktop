...
`shellDialect` selects the shell stack (`bash` default, `pwsh`): it picks the default `shellPath`/`shellArgs` (bash `--noprofile --norc -i`; pwsh `-NoLogo -NoProfile` through the shared `dsh-pwsh-local` resolver) and the startup contract. The bash dialect installs its prompt through the environment (`PS1` plus an OSC `133;D;`-terminated `PROMPT_COMMAND`). pwsh starts with an owned `-NoExit -EncodedCommand` argument that installs its `prompt` function and the shared `dsh-pwsh-local` UTF-8 encoding preamble before accepting input. Custom `shellArgs` configure the host; they must not supply a competing command or script. Startup waits within one absolute deadline for the OSC marker and printable prompt, or a standalone final prompt when constrained shells cannot emit the marker. Merely quoting the prompt source does not establish readiness. Its environment drops the bash-only markers and adds `NO_COLOR`. A viewport-sized `@xterm/headless` instance answers PowerShell/PSReadLine terminal queries, including cursor reports, without retaining a second scrollback. Replies go through the same subprocess terminal handle, propagate write failures, and drain during cleanup. Both dialects emit the same BEL-terminated OSC marker; the public output remains bounded, line-oriented text rather than a rendered terminal screen.

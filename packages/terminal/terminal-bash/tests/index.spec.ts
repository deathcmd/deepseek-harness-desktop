...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('does not publish pwsh readiness from startup text containing the prompt source', async () => {
    ...
    const ctx = new Context()
    const terminal = terminalHandle()
    const write = vi.fn(async (data: string) => { terminal.output.emit('data', data) })
  ...
  it.each([undefined, new AbortController().signal])('initializes pwsh once with its prompt, encoding, and caller signal (%s)', async (signal) => {
    const ctx = new Context()
    await ctx.plugin(SandboxPolicyService, { mode: 'danger-full-access', workspaceRoot: '/workspace' })
    let spawned: SubprocessTerminalSpawnSpec | undefined
    const initialize = vi.fn<LocalPtySession['initialize']>().mockResolvedValue(undefined)
    const session = stubLocalSession(initialize)
    const backend = new BashTerminalBackend(
      ctx,
      { ...config(), shellDialect: 'pwsh', shellPath: 'pwsh' },

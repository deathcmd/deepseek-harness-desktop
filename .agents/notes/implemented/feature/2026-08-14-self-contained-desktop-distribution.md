...
`apps/desktop-runtime` declares the official CLI as its single production root, so the desktop closure automatically follows the CLI's current bundle and plugin graph. The preparation script uses pnpm's legacy hoisted deployment layout, removes non-production source artifacts, and writes the link-free closure into a standalone `runtime.asar`. Native addons and subprocess helpers are kept beside it in `runtime.asar.unpacked`; a small external launcher imports the CLI from the archive with Electron's bundled Node.js runtime. No runtime link refers to the source checkout. Installed state and the NPM cache live under Electron's per-user data directory, while the initial workspace remains the user's home directory unless `DSH_DESKTOP_WORKSPACE` overrides it.
...
Electron Builder defines an interactive Windows NSIS installer and macOS DMG installers with an explicit product icon. The Windows installer stores its complete application payload in an uncompressed ZIP and writes it directly to the selected installation directory, avoiding the default 7z temporary-extract-and-copy pass. It records its process in a temporary launch lock before it replaces application files; a packaged desktop process refuses to start while that installer remains alive, so an executable written early in extraction cannot lock the remaining files. Completing installation removes the lock and broadcasts a lightweight shell association refresh. The fixed app ID and `deleteAppDataOnUninstall: false` make normal upgrades overwrite program files without removing Electron user data. Platform signing and notarization remain release-pipeline responsibilities.

## Verification

- The deployed runtime contains no symbolic links or Windows junctions; Electron's bundled Node.js imports the official CLI from `runtime.asar` and serves the Web root over loopback HTTP.
- The unpacked Windows application starts its packaged CLI after the temporary build-drive mapping is removed; closing the window leaves no Harness process tree.

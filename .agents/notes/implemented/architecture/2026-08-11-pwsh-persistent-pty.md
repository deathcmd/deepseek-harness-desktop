...
One backend, two dialects: `shellDialect: 'bash' | 'pwsh'` (default `'bash'`, existing deployments byte-identical). The effective `shellPath`/`shellArgs` resolve per dialect (bash `/bin/bash --noprofile --norc -i`; pwsh through the shared `dsh-pwsh-local` resolver with `-NoLogo -NoProfile`, keeping the interactive host for child REPLs). The child environment drops the bash-only `PS1`/`PROMPT_COMMAND` markers and adds `NO_COLOR` for pwsh. pwsh installs its prompt function and encoding preamble through an owned `-NoExit -EncodedCommand` argument, avoiding stdin echo and canonical-mode newline conversion before the console initializes. A viewport-only [headless terminal](https://github.com/xtermjs/xterm.js/blob/master/typings/xterm-headless.d.ts) answers cursor-position queries that PowerShell and PSReadLine issue even under `TERM=dumb`. Responses use the existing subprocess handle and participate in failure propagation and cleanup. A Linux stdin-wait syscall can describe that console query rather than readiness for new command input; pwsh therefore uses prompt and silence evidence on every platform, while bash retains the exact syscall tier. Startup retains one absolute deadline; output quoting the prompt source cannot establish readiness; the session waits for its OSC marker and exact printable tail, or a standalone final prompt before accepting markerless readiness. A `session_exit` or `timeout` wait rejects the spawn without renewing the deadline or resubmitting the bootstrap. Both dialects emit the same BEL-terminated OSC `133;D;` marker, so the sanitizer, `PROMPT_MARKER_PREFIX`, `CONTROLLED_PROMPT`, and the exact-tail readiness logic are reused untouched — the marker stays a readiness signal with an unconsumed payload, exactly as in the bash path, and no model-notification channel was added (aligned with the current implementation; the deferred BEL event channel stays deferred).
...
The terminal-bash configuration, lifecycle, and sanitizer suites run on Windows, including real pwsh sessions; only the real POSIX bash scenarios skip there. Its runtime sources remain coverage-required. Existing subprocess-local exemptions are independent, and the koffi-backed inspector stays excluded from Linux coverage. `tool-pwsh-persistent` is not exempt: its suite runs and its sources are coverage-required on the windows-native lane, mirroring `tool-bash-persistent`'s stub-mode matrix plus an echo-stripping mode; the real-pwsh suites prove persistent cwd/env, secret scrubbing, multiline and here-string commands, large-output clipping, and exit/reset over real ConPTY sessions. The ACP keyless snapshot boots the persistent tool through a real Loader composition and pins its model-visible schema and result.

## Alternatives considered

- **Hardcoded cursor replies or a noninteractive shell.** Rejected: a fixed `1;1` reply becomes false after output or cursor movement, while `-NonInteractive` removes `Read-Host`. The maintained headless parser owns cursor tracking and split escape sequences; the existing sanitizer still owns line-oriented output.
...
**Windows coverage includes the shared terminal backend.** `terminal-bash` and `tool-pwsh-persistent` run their platform-neutral suites and real pwsh scenarios on Windows. Prompt-source rejection, a no-prompt deadline, split cursor queries, reply failures, and reply-drain cases cover startup and protocol ownership; real pwsh also retains `Read-Host` input; complete Loader and ACP scenarios retain model-visible output checks.

**PowerShell readiness uses prompts and silence on every host.** The marker fast path covers shell prompts; markerless interactive input settles at the configured silence bound. The Linux exact stdin-wait tier remains available for bash.

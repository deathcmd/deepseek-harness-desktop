...
**Windows coverage includes the shared terminal backend.** `terminal-bash` and `tool-pwsh-persistent` run their platform-neutral suites and real pwsh scenarios on Windows. A bootstrap-echo regression and a no-prompt deadline case cover startup before a usable shell exists; complete Loader and ACP scenarios retain model-visible output checks.

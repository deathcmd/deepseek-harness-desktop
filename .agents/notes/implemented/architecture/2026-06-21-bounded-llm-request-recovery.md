...
- Controlled adapter timers pin the exact idle cutoff and SSE-comment rearming. The headless defaults snapshot retains real HTTP/SSE, request defaults, and final output under the normal provider deadline; it does not make subprocess scheduling meet a subsecond heartbeat budget. Removing comment rearming fails the controlled-timer test without relying on host load.

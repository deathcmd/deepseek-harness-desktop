...
如果你只是想使用，不需要安装 Node.js、pnpm 或命令行工具；请直接阅读下面的“下载”和“安装”部分。只有要参与开发或自己打包时，才需要阅读文末的源码构建说明。
...
- 安装时保持网络可用，并预留至少 1 GB 空间。安装包约 113 MB，解压后的运行时会比下载文件大。
...
安装器会在写入文件期间阻止应用提前启动。这样做是为了防止程序只解压了一部分就被打开，造成安装卡住或文件被占用；如果看到“仍在安装”的提示，回到安装器等待它完成即可。
...
```text
F1858EBBA2A1C3340703D65B5BBD5704705488E1664B1612C6B21F087C77A36D
```
...
确认安装器窗口仍然显示运行中，并且没有同时打开桌面快捷方式。等待安装器明确结束；如果仍无变化，取消后重新运行安装程序，并换一个较短、可写的目录，例如 `D:\DeepSeek Harness Desktop`。不要在安装未完成时反复双击 EXE。
